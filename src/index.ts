...
class msaiagentService {
  ...
  private initializeRoutes(): void {
    ...
    this.app.use('/api/v1/aiagents', aiagentRoutes);
    this.app.use('/api/v1/chat', chatRoutes);

    // Root endpoint
    this.app.get('/', (req, res) => {
      res.json({
        service: 'ms-ai-agent Service',
        version: '1.0.0',
        status: 'running',
        timestamp: new Date().toISOString(),
  ...
  private initializeSwagger(): void {
    const options = {
      definition: {
        openapi: '3.0.0',
        info: {
          title: 'ms-ai-agent API',
          version: '1.0.0',
          description: 'API para ms-ai-agent',
          contact: {
            name: 'Lazarus Team',
    ...
    this.app.use('/api-docs', swaggerUi.serve, swaggerUi.setup(specs, {
      ...
      customSiteTitle: 'ms-ai-agent API Documentation'
    }));
  }
  ...
  public async start(): Promise<void> {
    try {
      ...
      this.app.listen(this.port, '0.0.0.0', () => {
        logger.info(`ms-ai-agent service running on port ${this.port}`);
  ...
  public async stop(): Promise<void> {
    logger.info('Stopping ms-ai-agent service...');
    process.exit(0);
  }
}

// Start the service
const service = new msaiagentService();
service.start();
